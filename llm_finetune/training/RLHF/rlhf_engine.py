...
class DeepSpeedRLHFEngine():
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        ds_config = get_train_ds_config(
            ...
            loss_initial_scale_power=self.args.actor_loss_initial_scale_power,
        ...
        if self.args.actor_lora_dim > 0:
            actor_model = convert_linear_layer_to_lora(
                ...
                self.args.actor_lora_dim, self.args.actor_lora_alpha)
            if self.args.load_actor_lora_dir is not None:
                load_lora_weights(actor_model, self.args.load_actor_lora_dir, self.args.actor_zero_stage)
    ...
    def _init_ref(self, actor_model_name_or_path):
        ...
        ref_model = create_hf_model(AutoModelForCausalLM,
                                    ...
                                    ds_config)
        if self.args.load_actor_lora_dir is not None:
            convert_linear_layer_to_lora_by_cfg(ref_model, self.args.load_actor_lora_dir)
            load_lora_weights(ref_model, self.args.load_actor_lora_dir, zero_stage)
    ...
    def _init_ema(self, actor_model_name_or_path):
        ...
        if self.args.actor_lora_dim > 0:
            actor_model_ema = convert_linear_layer_to_lora(
                ...
                self.args.actor_lora_dim, self.args.actor_lora_alpha)
            if self.args.load_actor_lora_dir is not None:
                load_lora_weights(actor_model_ema, self.args.load_actor_lora_dir, zero_stage)
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        ds_config = get_train_ds_config(
            ...
            enable_tensorboard=self.args.enable_tensorboard,
            loss_initial_scale_power=self.args.critic_loss_initial_scale_power,
            tb_path=self.args.tensorboard_path,
        ...
        if self.args.critic_lora_dim > 0:
            critic_model = convert_linear_layer_to_lora(
                ...
                self.args.critic_lora_dim, self.args.critic_lora_alpha)
            if self.args.load_actor_lora_dir is not None:
                load_lora_weights(critic_model, self.args.load_critic_lora_dir, self.args.critic_zero_stage)
            if self.args.only_optimize_lora:
    ...
    def _init_reward(self, critic_model_name_or_path):
        ...
        if self.args.load_critic_lora_dir is not None:
            convert_linear_layer_to_lora_by_cfg(reward_model, self.args.load_critic_lora_dir)
            load_lora_weights(reward_model, self.args.load_critic_lora_dir, zero_stage)
            convert_lora_to_linear_layer(reward_model)
